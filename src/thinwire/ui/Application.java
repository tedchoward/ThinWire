 ...
 * Methods in this class are those that directly affect the system and it's
 * environment.
 * 
...
public abstract class Application {
	private static final Logger log = Logger.getLogger(Application.class
			.getName());
	private static final String DEFAULT_STYLE_SHEET = "class:///"
			+ Application.class.getName() + "/resources/CorporateStyle.zip";
	...
	private long synchInterval = 500;// Interval between synchronization
	private Runnable syncRunnable = new Runnable() {
		public void run() {
			while (syncedExecs.size() > 0) {
				try {
					syncedExecs.removeFirst().run();
				} catch (Exception e) {
					e.printStackTrace();
					System.err.println("synchronized call failed");
				}
			}
	...
	LinkedList<Runnable> syncedExecs = new LinkedList<Runnable>();
	static {
		Properties props = new Properties();
		Map<String, String> vi = new HashMap<String, String>();

		try {
			props.load(Application.class
					.getResourceAsStream("resources/versionInfo.properties"));
			...
			defaultStyleMap = buildStyleMap(Application.class.getClassLoader(),
					xod);
		} catch (IOException e) {
	...
	public static class Local<T> {
		...
		public T get() {
			...
			if (value == null && !map.containsKey(this))
	...
	/**
	 * Returns the current version info details for the platform. The returned
	 * Map is read-only and contains the following keys: companyName
	 * companyAddress1 companyAddress2 companyCity companyState companyZip
	 * companyPhone companyWebsite productDescription productVersion
	...
	public static void main(String[] args) throws Exception {
		...
		BufferedReader r = new BufferedReader(new InputStreamReader(
				Application.class
						.getResourceAsStream("resources/licenseHeader.txt")));
		...
		final java.awt.Frame frame = new java.awt.Frame(
				"ThinWire(R) Ajax RIA Framework v"
						+ getPlatformVersionInfo().get("productVersion"));
	...
	/**
	 * @return the current instance of the application, or null if called from a
	...
	thinwire.render.ComponentRenderer getRenderer(Class clazz) {
		Class<thinwire.render.ComponentRenderer> rendererClass = Renderer.RendererManager
				.getInstance(getPlatform()).getRenderer(clazz);
		if (rendererClass == null) {
			if (clazz.getSuperclass() != AbstractComponent.class) {
				return getRenderer(clazz.getSuperclass());
			} else {
				throw new RuntimeException("No renderer for component class '"
		...
		try {
			...
			throw new RuntimeException(
					"Illegal acces Exception thrown while getting renderer for '"
							+ clazz.getCanonicalName() + "'.");
		}
	}

	/**
	 * Returns an InputStream representing the specified resource. The following
	 * URL's are supported:
	 * <ul>
	 * <li>Any valid resource filename. If Application.current() returns
	 * non-null, which is the typical scenario, then relative paths are
	 * interpreted as relative to the application folder.
	 * {@link #getRelativeFile}</li>
	 * <li>class:///com.mypackage.MyClass/resource.ext</li>
	 * <li>http://www.mycompany.com/resource.ext</li>
	 * </ul>
	 * 
	 * @return an InputStream representing the specified resource or null if the
	...
	protected static InputStream getResourceAsStream(ClassLoader classLoader,
			String uri) {
		InputStream is = null;
			...
			try {
				...
				if (uri.startsWith("class:///")) {
					...
					if (classLoader == null) {
						...
						classLoader = app == null ? Application.class
								.getClassLoader() : app.getClassLoader();
					}

					is = classLoader.loadClass(className).getResourceAsStream(
							resource);
				} else if (uri.indexOf("://") >= 0
						&& !uri.startsWith("file:///")) {
					URL remoteImageURL = new URL(uri);
					URLConnection remoteImageConnection = remoteImageURL
							.openConnection();
					...
					if (innerFile != null) {
						...
						while ((entry = zip.getNextEntry()) != null) {
							if (!entry.isDirectory()
									&& entry.getName().toLowerCase().equals(
											innerFile)) {
								byte[] bytes = new byte[(int) entry.getSize()];
								int pos = 0, cnt;

								while ((cnt = zip.read(bytes, pos, bytes.length
										- pos)) > 0) {
									pos += cnt;
								}
	...
	public static void writeResourceToStream(String uri, OutputStream os) {
		...
		if (is == null)
			throw new IllegalArgumentException("Content for URI was not found:"
					+ uri);
	...
	// XXX: This method exposes a "potential" security issue because it expose
	// the ClassLoader & Classes of the app to non-app libraries.
	@SuppressWarnings("unchecked")
	public static Class getApplicationContextClass(String className)
			throws ClassNotFoundException {
		Application app = current();
		return (app == null ? Application.class.getClassLoader() : app
	...
	public static Style getDefaultStyle(Class<? extends Component> clazz) {
		...
		Map<Class<? extends Component>, Style> map = current() != null ? current().compTypeToStyle
				: defaultStyleMap;
		if (map == null)
			map = defaultStyleMap;
		...
		if (style == null) {
			...
			do {
				...
				if (style == null) {
					...
					if (sc != null && Component.class.isAssignableFrom(sc))
						lst.add(sc);

					for (Class i : findClazz.getInterfaces()) {
						if (Component.class.isAssignableFrom(i))
							lst.add(i);
					}
				} else {
					break;
	...
	@SuppressWarnings("unchecked")
	private static Map<Class<? extends Component>, Style> buildStyleMap(
			ClassLoader classLoader, XOD props) {
			...
			if (value instanceof Style) {
				if (name.startsWith("default")) {
					if (name.equals("default"))
						styleMap.put(null, (Style) value);
				} else if (name.matches(".*?[A-Z].*?")) {
					if (Character.isUpperCase(name.charAt(0))
							&& name.indexOf('.') == -1)
						name = "thinwire.ui." + name;

					try {
						Class clazz = classLoader.loadClass(name);
						if (clazz.getMethod("getStyle") != null)
							styleMap.put((Class<? extends Component>) clazz,
									(Style) value);
				...
				throw new UnsupportedOperationException(
						"Unsupported object type in style sheet:"
								+ value.getClass());
			}
		}
	...
	protected void shutdown() {
		...
		systemColors.clear();
		systemImages.clear();
		if (compTypeToStyle != null)
			compTypeToStyle.clear();
	...
	@SuppressWarnings("unchecked")
	<T extends EventListener> EventListenerImpl<T> getGlobalListenerSet(
			Class<T> type, boolean createIfNull) {
		if (globalListeners == null) {
			if (createIfNull) {
		...
		EventListenerImpl<T> set = (EventListenerImpl<T>) globalListeners
				.get(type);
		if (set == null && createIfNull)
			globalListeners.put(type,
					set = new EventListenerImpl<T>(null, type));
		return set;
	}

	/**
	 * Adds a <code>PropertyChangeListener</code> that will be notified when the
	 * specified property of any new component changes. To further clarify, a
	 * global property change listener will receive an event notification for
	 * any component that is created after this global property change listener
	 * is added, for which the specified property changes. Therefore,
	 * establishing a global property change listener is the same as invoking
	 * the
	 * {@link Component#addPropertyChangeListener(String, PropertyChangeListener)}
	 * method on every component after it's creation.
	 * <p>
	 * As a general rule, you should avoid using this feature because it can
	 * cause a large volume of events to be generated. This is especially true
	 * if you listen to a frequently updated property, such as
	 * <code>PROPERTY_TEXT</code>. However, there are cases where this can be
	 * quite useful. One such case is when you use the 'userObject' property to
	 * store a boolean state that indicates whether a value is required for the
	 * component. In such a case, you can establish a global property change
	 * listener for 'userObject' and then based on the property being set to
	 * <code>true</code> you could update the background color so it is apparent
	 * to the user that a value is required.
	 * </p>
	 * <b>Example:</b>
	 * 
	 * <pre>
	 * Component.addGlobalPropertyChangeListener(Component.PROPERTY_USER_OBJECT,
	 * 		new PropertyChangeListener() {
	 * 			public void propertyChange(PropertyChangeEvent pce) {
	 * 				Component comp = (Component) pce.getSource();
	 * 
	 * 				if (comp.getUserObject() == Boolean.TRUE) {
	 * 					comp.getStyle().getBackground().setColor(
	 * 							Color.PALEGOLDENROD);
	 * 				} else {
	 ...
	 * 
	 * @param propertyName
	 *            the name of the property that the listener will receive change
	 *            events for.
	 * @param listener
	 *            the listener that will receive
	 *            <code>PropertyChangeEvent</code> objects upon the property of
	 *            any new component changing.
	 * @throws IllegalArgumentException
	 *             if <code>listener</code> or <code>propertyName</code> is null
	 *             or if <code>propertyName</code> is an empty string.
	...
	public void addGlobalPropertyChangeListener(String propertyName,
		...
		EventListenerImpl<PropertyChangeListener> set = getGlobalListenerSet(
				PropertyChangeListener.class, true);
		set.addListener(propertyName, listener);
	}

	/**
	 * Adds a <code>PropertyChangeListener</code> that will be notified when any
	 * of the specified properties of any new component changes. This method is
	 * equivalent to calling
	 * {@link #addGlobalPropertyChangeListener(String, PropertyChangeListener)}
	 * once for each property you want to listen to.
	 * 
	 * @param propertyNames
	 *            a string array of property names that the listener will
	 *            receive change events for.
	 * @param listener
	 *            the listerner that will receive
	 *            <code>PropertyChangeEvent</code> objects anytime one of the
	 *            specified propertyNames of any new component change.
	 * @throws IllegalArgumentException
	 *             if <code>listener</code>, <code>propertyNames</code> or any
	 *             property name is the array is null or if any property name is
	 *             an empty string.
	 * @see #addGlobalPropertyChangeListener(String, PropertyChangeListener)
	 * @see Component#addPropertyChangeListener(String[],
	 *      PropertyChangeListener)
	 * @see thinwire.ui.event.PropertyChangeListener
	 * @see thinwire.ui.event.PropertyChangeEvent
	 */
	public void addGlobalPropertyChangeListener(String[] propertyNames,
			PropertyChangeListener listener) {
		EventListenerImpl<PropertyChangeListener> set = getGlobalListenerSet(
				PropertyChangeListener.class, true);
	...
	/**
	 * Removes the <code>PropertyChangeListener</code> from the list of global
	 * listeners that are added to all new <code>Component</code>'s. To further
	 * clarify, removing a global property change listener will NOT remove the
	 * <code>listener</code> from <code>Component</code>'s that have already
	 * been created.
	 * 
	 * @param listener
	 ...
	 * @see thinwire.ui.event.PropertyChangeListener
	 */
	public void removeGlobalPropertyChangeListener(
			PropertyChangeListener listener) {
		EventListenerImpl<PropertyChangeListener> set = getGlobalListenerSet(
				PropertyChangeListener.class, true);
	...
	/**
	 * Adds a <code>ActionListener</code> that will be notified when the
	 * specified action occurs on any new component. To further clarify, a
	 * global action listener will receive an event notification for any
	 * component that is created after this global action listener is added, on
	 * which the specified action occurs. Therefore, establishing a global
	 * action listener is the same as invoking the
	 * {@link Component#addActionListener(String, ActionListener)} method on
	 * every component after it's creation.
	 * 
	 * @param action
	 *            the action to specficially be notified of.
	 * @param listener
	 *            the event listener that will receive notification.
	 */
	public void addGlobalActionListener(String action, ActionListener listener) {
		EventListenerImpl<ActionListener> set = getGlobalListenerSet(
				ActionListener.class, true);
		set.addListener(action, listener);
	}

	/**
	 * Adds a <code>ActionListener</code> that will be notified when any of the
	 * specified actions occur on any new component. This method is equivalent
	 * to calling {@link #addGlobalActionListener(String, ActionListener)} once
	 * for each action you want to receive notification for.
	 * 
	 * @param actions
	 *            the actions to specficially be notified of.
	 * @param listener
	 *            the event listener that will receive notifications.
	 */
	public void addGlobalActionListener(String[] actions,
	...
	/**
	 * Unregister an <code>ActionListener</code> from all action event
	 * notifications from any component.
	 * 
	 * @param listener
	 *            the listener that should no longer receive action event
	 *            notifications.
	 */
	...
	public void addGlobalDropListener(Component dragSource,
		...
		EventListenerImpl<DropListener> set = getGlobalListenerSet(
				DropListener.class, true);
	...
	public void addGlobalKeyPressListener(String keyPressCombo,
			KeyPressListener listener) {
		EventListenerImpl<KeyPressListener> set = getGlobalListenerSet(
				KeyPressListener.class, true);
		set.addListener(keyPressCombo, listener);
	}

	public void addGlobalKeyPressListener(String[] keyPressCombos,
			KeyPressListener listener) {
		EventListenerImpl<KeyPressListener> set = getGlobalListenerSet(
				KeyPressListener.class, true);
	...
	public void addGlobalItemChangeListener(ItemChangeListener listener) {
		EventListenerImpl<ItemChangeListener> set = getGlobalListenerSet(
				ItemChangeListener.class, true);
	...
	public void reportException(Object source, Throwable exception) {
		...
		for (ExceptionListener el : exceptionListeners
				.toArray(new ExceptionListener[exceptionListeners.size()])) {
			el.exceptionOccurred(ee);
			if (ee.isStopPropagation())
				break;
		}

		if (!ee.isSuppressLogging())
		...
		if (!ee.isCanceled()) {
			...
			taUserMessage.setBounds(img == null ? gap : img.getX()
					+ img.getWidth() + gap, gap, msgSize, 60);
			...
			bDetail.setPosition(taUserMessage.getX() + taUserMessage.getWidth()
					- bDetail.getWidth(), taUserMessage.getY()
					+ taUserMessage.getHeight() + gap);
			d.getChildren().add(bDetail);

			final TextArea taDetail = new TextArea(ee.getStackTraceText());

			final Button bOk = new Button();
			bOk.setBounds(bDetail.getX() - gap - bDetail.getWidth(), bDetail
					.getY(), bDetail.getWidth(), bDetail.getHeight());
			bOk.setText("OK");
			...
			taDetail.setBounds(taUserMessage.getX(), bOk.getY()
					+ bOk.getHeight() + gap, msgSize * 2, msgSize);
			taDetail.setVisible(false);
			d.getChildren().add(taDetail);

			int x = (getFrame().getInnerWidth() / 2)
					- ((d.getWidth() + msgSize) / 2);
			int y = (getFrame().getInnerHeight() / 2)
					- ((d.getHeight() + msgSize) / 2);
			if (x < 0)
				x = 0;
			if (y < 0)
				y = 0;
			...
			final int normalWidth = 6 + taUserMessage.getX()
					+ taUserMessage.getWidth() + gap;
			final int normalHeight = 25 + bDetail.getY() + bDetail.getHeight()
					+ gap;
			...
			final PropertyChangeListener dialogSizePCL = new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent pce) {
					int diff = ((Integer) pce.getNewValue())
							- ((Integer) pce.getOldValue());
			...
			bDetail.addActionListener(Button.ACTION_CLICK,
					new ActionListener() {
						public void actionPerformed(ActionEvent ev) {
							if (bDetail.getText().equals("Show Details >>")) {
								...
								d
										.addPropertyChangeListener(
												new String[] {
														Dialog.PROPERTY_WIDTH,
														Dialog.PROPERTY_HEIGHT },
												dialogSizePCL);
							} else if (bDetail.getText().equals(
									"<< Hide Details")) {
	 ...
	 * window.
	 * 
	 * @return the <code>Frame</code> that represents the primary application
	 *         window.
	 */
	public Frame getFrame() {
		if (frame == null) {
			frame = new Frame();
			frame.addPropertyChangeListener(new String[] { "width", "height" },
					new PropertyChangeListener() {
						public void propertyChange(PropertyChangeEvent ev) {
							layoutMinimizedDialogs();
						}
					});
			removeTimerTask(syncRunnable);
			setSynchInterval(synchInterval);
		}
		return frame;
	}
	 ...
	 * Application. The <code>Component</code> that previously had focus is the
	 * last <code>Component</code> in ANY window that held the focus. This is
	 * different from the behavior of the focus property in which their is one
	 * <code>Component</code> with focus PER window.
	 * 
	 * @return the <code>Component</code> that previously had focus in the
	 *         Application, or null if no <code>Component</code> has had prior
	 *         focus.
	...
	protected void loadStyleSheet(String styleSheet) {
		...
		for (Map.Entry<String, Object> e : props.getObjectMap().entrySet()) {
			...
			if (value instanceof Color) {
				Color color = Color.valueOf(name); // Just verify that the
													// specified systemColor is
													// valid;
				if (!color.isSystemColor())
					throw new UnsupportedOperationException(
							"You can only override system colors in the style sheet: "
									+ name);
		...
		for (Color c : Color.values()) {
			if (c.isSystemColor()) {
				...
				if (systemColors.get(name) == null)
					systemColors.put(name, c);
			}
		}
	...
	protected String getSystemFile(String value) {
		if (!value.matches("^\\w{3,}://.*")) {
			if (value != null && !value.matches("^\\w?:?[\\\\|/].*"))
				value = getBaseFolder() + File.separator + value;
	...
	public static File getRelativeFile(String pathname) {
		...
		return app == null ? new File(pathname) : new File(app
				.appendBaseFolder(pathname));
	}
	...
	public static File getRelativeFile(String parent, String child) {
		Application app = Application.current();
		return app == null ? new File(parent, child) : new File(app
				.appendBaseFolder(parent), child);
	}

	protected Object setPackagePrivateMember(String memberName, Component comp,
			Object value) {
	...
	private String appendBaseFolder(String s) {
		...
		if (s != null && !s.matches("^\\w?:?[\\\\|/].*")) {
			...
			// This lowers the chance of someone using this resourceMap to
			// override a valid
			// file path. i.e. map.put("C:\\WORK\\CUSTOMERFILE.DOC",
			// "C:\\TEMP\\HACK.TXT")
			// would make new Resource("C:\\WORK\\CUSTOMERFILE.DOC") return a
			// file reference
			...
			// #ENDIF
			if (ret == null)
				ret = getBaseFolder() + File.separator + s;
		}
	...
	/**
	 * @return
	 * @deprecated there is no replacement for this method, instead use String
	 *             constant variables.
	 */
	...
	public void syncExec(Runnable what){
		this.syncedExecs.add(what);
	}
	public void setSynchInterval(long synchInterval) {
		this.synchInterval = synchInterval;

		try {
			removeTimerTask(syncRunnable);
			addTimerTask(syncRunnable, synchInterval,true);
